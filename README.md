...
## 📍 프로젝트 목표
1. Redis 활용<br>CDN 서버에서 이미지를 캐시하여 조회 성능을 향상시키고, 이미지 데이터의 빠른 접근 가능
2. Kafka 활용<br>이미지 업로드를 비동기적으로 처리하여 성능을 향상시키고, 원본 이미지 및 리사이징 이미지를 효율적으로 업로드 서버로 전달하여 처리 시간을 단축
...
- **Minio 서버 접근 최소화**<br> 이미지 변환, 리사이징, 메타데이터 제거 과정에서 불필요한 Minio 접근을 Redis 캐싱으로 대체하여 업로드 처리 시간을 감소
- **Kafka 비동기 실행**<br> Kafka를 통해 변환된 이미지 정보를 전달하고, 원본 업로드와 리사이징 업로드가 **비동기적으로** 실행

### **성능 개선 전**
- **업로드 시간**: 1077ms  
  동일한 이미지를 업로드하는 데 **1077ms**가 소요

### **성능 개선 후**
- **업로드 시간**: 580ms  
  성능 개선 후 동일한 이미지를 업로드하는 데 **580ms**가 소요
...
### **성능 향상 요인**
- **레디스 캐싱 방식 변경** <br>이전에는 파일을 다운로드하고 Redis에 CDN URL 경로를 저장하는 방식, 이는 용량을 차지하고 성능 저하 발생 <br>개선 후, 이미지 조회 시 Redis에 데이터가 없으면 Minio에서 조회한 후 Redis에 CDN URL과 이미지를 저장하여 성능을 향상
...
- **개선 후**: 이미지 데이터를 **Redis에 캐시**하여 조회 속도를 개선, **응답 시간**이 대폭 단축
